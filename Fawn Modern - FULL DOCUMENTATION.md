...
**Version:** `10.1.0-serotonin`  
**API revision:** `50`  
**Build:** `fawn-modern-10.1.0-accessory-keybind-live-20260917`
...
| `ESPPreview:GetEnabled()` | Returns whether the preview participates in rendering/layout. |
| `ESPPreview:GetSilhouette()` | Returns whether the clean player silhouette is shown. |
| `ESPPreview:GetStageGuides()` | Returns whether subtle stage guides are shown. |
...
| `ESPPreview:SetEnabled(v)` | Fully enables/disables preview rendering and dock layout participation. |
...
| `ESPPreview:SetSilhouette(v)` | Shows/hides the clean player silhouette. |
| `ESPPreview:SetStageGuides(v)` | Shows/hides subtle stage guide lines. |
...
### Three-dot keybind behavior

10.1.0 rewrites keybind interaction inside the three-dot accessory menu so both keybind sources behave the same way:

1. The **first built-in Keybind row** created by a parent toggle's `keybind={...}` option.
2. Any custom `AddKeybind(...)` / `AddAccessoryKeybind(...)` control inside `Control:Extra(...)`.

Both now use the same interaction rules:

- **Left-click the key field** to begin key capture.
- Press a keyboard key or mouse button to assign it.
- **Backspace** clears the captured bind.
- **Escape** cancels capture.
- **Click the dots** beside the field, or **right-click the key field**, to open the mode menu.
- Mode menu options are `Hold`, `Toggle`, `Always`, `Clear`, and `Close`.
- The mode popup is a dedicated child of the three-dot menu, so clicks cannot be swallowed by or click through the accessory shell.

```lua
local Owner=Section:AddToggle("Feature",true,{
    keybind={
        key="G",
        label="G",
        mode="Toggle",
...
Owner:Extra(function(extra)
    extra:AddKeybind("Secondary Key","H",{
        mode="Hold",
        keybindList=true
    })
end)
...
The first `G` bind and the custom `H` bind are both fully editable from the same three-dot menu.

### Live Keybind List support for accessories

`Window:ForEachControl()` now recursively walks `extraSection` controls created by `Control:Extra()` and the `AddAccessory*` helpers. This fixes two related issues:

- accessory keybinds now participate in normal per-frame keybind processing, so Hold/Toggle/Always activation works;
- automatic `KeybindList` overlays discover them and update their key, mode, and active state live.

```lua
local Keys=W:KeybindList({
    title="KEYBINDS",
    auto=true,
    onlyActive=false
})
...
If an accessory key changes from `H` to `J`, the overlay reads the same keybind object on the next render and shows `J` immediately. Changing its mode to `Always` also updates immediately.
...
1. Open a parent control's three-dot menu.
2. Open an accessory dropdown whose popup overlaps another accessory control.
3. Select the second dropdown option and confirm the overlapped control is unchanged.
4. Open the accessory color picker and click the SV area; confirm the RGBA value changes.
5. Close the color picker by clicking its color chip again.
6. Click the accessory slider value box, erase the existing value, type `250`, and press Enter while the visual slider range remains `0–100`.
7. Click the same outer three-dot trigger again and confirm the entire accessory menu closes without reopening on that same click.
